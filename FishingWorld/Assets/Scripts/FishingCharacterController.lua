...
--!SerializeField
local fishingIndicator : GameObject = nil
...
local fishingIndicatorInstance : GameObject? = nil
...
---
--- Show the fishing indicator at a given position
---
function ShowFishingIndicator(point : Vector3)
	-- Create the fishing indicator if we have not created it yet
	if not fishingIndicatorInstance and fishingIndicator then
		fishingIndicatorInstance = Object.Instantiate(fishingIndicator) :: GameObject?
		fishingIndicatorInstance:SetActive(false)
	end
	
	if fishingIndicatorInstance then
		fishingIndicatorInstance.transform.position = point
		--fishingIndicatorInstance:SetActive(true)
	end
end
---
--- Hide the fishing indicator
function HideFishingIndicator()
	if fishingIndicatorInstance then
		fishingIndicatorInstance:SetActive(false)
	end
end
...
local function LocalMoveTo(
	...
	elseif distance > 0 then
		...
		moved = character:MoveTo(point, areaMask, callback :: (any) -> (any), 8)
	end
...
local function HandleTap(tap : TapEvent)
	...
	if hit.collider.gameObject.tag == "Water" then
		...
		local distFromAvatar = Vector3.Distance(character.transform.position, hit.point)
